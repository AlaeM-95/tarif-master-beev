...
// Avantage ou remise collaborateur (facultatif) : libellé + détail.
export type BpuAvantage = { label: string; detail: string };
...
export type BpuB2B2EConfig = {
  ...
  bornes: BpuBorne[];
  avantages: BpuAvantage[]; // remises / avantages collaborateur (facultatif)
};

export function newBpuAvantage(): BpuAvantage { return { label: "Nouvel avantage", detail: "" }; }
...
// ─── Présentation (rebrand 2026) ───────────────────────────────────────────
// Charte Beev : fond beige, encres ink, accents rose/bleu, logo Beev embarqué.

export type BpuAssets = { logoDark?: string; logoLight?: string };

function headLogo(assets?: BpuAssets): string {
  return assets?.logoDark
    ? `<img class="head-logo" src="${assets.logoDark}" alt="Beev" />`
    : `<span class="head-mark">beev</span>`;
}

function pageHead(numStr: string, borneName: string, assets?: BpuAssets): string {
  return `<div class="page-head">
    <div class="page-head-l"><span class="page-num-tag">${esc(numStr)}</span><span class="page-borne">${esc(borneName)}</span></div>
    ${headLogo(assets)}
  </div>`;
}

function coverPage(cfg: BpuB2B2EConfig, assets?: BpuAssets): string {
  const sub = cfg.clientName ? `${esc(cfg.clientName)} × Beev · ${esc(cfg.year)}` : `Partenariat × Beev · ${esc(cfg.year)}`;
  ...
  const beevLogo = assets?.logoDark ? `<img class="cover-beev" src="${assets.logoDark}" alt="Beev" />` : `<div class="beev-mark">beev</div>`;
  return `
...
function equipPage(b: BpuBorne, num: number, running: string, assets?: BpuAssets): string {
  ...
  return `
  <section class="page">
    ${pageHead(numStr, `Borne ${b.name}`, assets)}
    <div class="equip-top">
    ...
    <div class="page-foot"><span>${esc(running)}</span></div>
  </section>`;
}

function installPage(b: BpuBorne, num: number, running: string, assets?: BpuAssets): string {
  const numStr = String(num).padStart(2, "0");
  ...
  <section class="page">
    ...
    <div class="forfait-note">${esc(b.forfaitBase || "")}</div>
...
function avantagesPage(cfg: BpuB2B2EConfig, running: string, assets?: BpuAssets): string {
  const items = (cfg.avantages || []).filter((a) => a.label.trim());
  if (!items.length) return "";
  const cards = items.map((a, i) => `
    <div class="adv-card">
      <span class="adv-num">${String(i + 1).padStart(2, "0")}</span>
      <div class="adv-body">
        <div class="adv-title">${esc(a.label)}</div>
        ${a.detail ? `<div class="adv-text">${esc(a.detail)}</div>` : ""}
      </div>
    </div>`).join("");
  return `
  <section class="page">
    ${pageHead("✦", "Avantages collaborateur", assets)}
    <h2 class="section-title">Avantages & remises collaborateur</h2>
    <div class="adv-intro">Conditions préférentielles accordées dans le cadre du partenariat.</div>
    <div class="adv-grid">${cards}</div>
    <div class="page-foot"><span>${esc(running)}</span></div>
  </section>`;
}

function closingPage(cfg: BpuB2B2EConfig, assets?: BpuAssets): string {
  const beevLogo = assets?.logoLight ? `<img class="closing-logo" src="${assets.logoLight}" alt="Beev" />` : `<div class="beev-mark light">beev</div>`;
  return `
...
export function buildBpuB2B2EHtml(cfg: BpuB2B2EConfig, fonts?: RoobertFonts, assets?: BpuAssets): string {
  ...
  const pages: string[] = [coverPage(cfg, assets)];
  bornes.forEach((b, i) => {
    pages.push(equipPage(b, i + 1, running, assets));
    pages.push(installPage(b, i + 1, running, assets));
  });
  const adv = avantagesPage(cfg, running, assets);
  if (adv) pages.push(adv);
  pages.push(closingPage(cfg, assets));
...
<style>
${fontFaceCss(fonts)}
  :root { --ink:#1A1A1A; --beige:#FBF8F1; --paper:#FFFFFF; --rose:#F4B8AA; --bleu:#A5D2FF; --violet:#D3CCD8; --vert:#1FA463; --sub:#6A6A6F; --rule:#E8E4DC; }
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .page { position: relative; width: 210mm; min-height: 297mm; padding: 20mm 18mm; background: #fff; page-break-after: always; overflow: hidden; }
  ...
  .partner-label { font-size: 10px; letter-spacing: .2em; text-transform: uppercase; color: var(--sub); font-weight: 600; }
  .client-logo { max-height: 54px; max-width: 220px; object-fit: contain; }
  .client-logo-text { font-size: 22px; font-weight: 700; }
  .cover-beev { height: 30px; width: auto; object-fit: contain; }
  .beev-mark { font-size: 24px; font-weight: 700; letter-spacing: -.02em; }
  .beev-mark.light { color: var(--beige); }
  ...
  .cover-title { font-size: 62px; line-height: 1.02; font-weight: 700; letter-spacing: -.025em; }
  .cover-rule { display: block; width: 64px; height: 5px; border-radius: 3px; background: var(--rose); margin: 26px 0; }
  .cover-sub { font-size: 20px; font-weight: 700; margin-bottom: 16px; }
  ...
  .page-head { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 26px; }
  .page-head-l { display: flex; align-items: center; gap: 12px; }
  .page-num-tag { font-size: 13px; font-weight: 700; color: var(--ink); background: var(--rose); border-radius: 7px; padding: 3px 10px; }
  .page-borne { font-size: 12px; letter-spacing: .14em; text-transform: uppercase; font-weight: 600; color: var(--sub); }
  .head-logo { height: 20px; width: auto; object-fit: contain; }
  .head-mark { font-size: 16px; font-weight: 700; }
  .section-title { font-size: 30px; font-weight: 700; letter-spacing: -.02em; margin-bottom: 22px; }
  ...
  .product-img { flex-shrink: 0; width: 200px; height: 130px; background: var(--beige); border-radius: 14px; display: flex; align-items: center; justify-content: center; overflow: hidden; margin-top: -6px; }
  .product-img img { max-width: 88%; max-height: 88%; object-fit: contain; }

  .equip-table { width: 100%; border-collapse: collapse; margin-bottom: 30px; border: 1px solid var(--rule); border-radius: 12px; overflow: hidden; }
  .equip-table th { text-align: left; font-size: 10px; letter-spacing: .1em; text-transform: uppercase; color: var(--sub); font-weight: 600; padding: 12px 14px; background: #F6F2E9; border-bottom: 1px solid var(--rule); }
  .equip-table th.r, .equip-table td.r { text-align: right; }
  .equip-table td { font-size: 14px; padding: 13px 14px; border-bottom: 1px solid var(--rule); }
  .equip-table tr:last-child td { border-bottom: none; }
  .equip-table tr.total td { font-weight: 700; background: #FBF8F1; }

  .specs-block { background: var(--beige); border-radius: 14px; padding: 20px 22px; }
  ...
  .install-card { border-radius: 14px; padding: 20px 22px; border: 1px solid var(--rule); }
  .install-card.rose { background: #FDF1EE; border-color: #F4D9D1; }
  .install-card.bleu { background: #EDF6FF; border-color: #CFE6FB; }
  ...
  .adv-intro { font-size: 12px; color: var(--sub); margin: -10px 0 20px; }
  .adv-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }
  .adv-card { display: flex; gap: 14px; background: var(--paper); border: 1px solid var(--rule); border-left: 4px solid var(--vert); border-radius: 12px; padding: 16px 18px; }
  .adv-num { font-size: 16px; font-weight: 700; color: var(--vert); line-height: 1; }
  .adv-title { font-size: 14px; font-weight: 700; margin-bottom: 4px; }
  .adv-text { font-size: 12px; color: var(--sub); line-height: 1.5; }

  .page-foot { position: absolute; left: 18mm; right: 18mm; bottom: 14mm; font-size: 9px; color: var(--sub); border-top: 1px solid var(--rule); padding-top: 8px; }

  .closing { background: var(--ink); color: var(--beige); display: flex; flex-direction: column; }
  .closing .cover-foot { color: rgba(251,248,241,.6); border-top-color: rgba(251,248,241,.18); }
  .closing-center { flex: 1; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; gap: 22px; }
  .closing-logo { height: 34px; width: auto; object-fit: contain; }
  .closing-validity { font-size: 13px; color: rgba(251,248,241,.7); }
  .closing-tagline { font-size: 48px; font-weight: 700; letter-spacing: -.025em; }
  ...
  @media screen { body { background: #E9E6DF; padding: 24px 0; } .page { margin: 0 auto 24px; box-shadow: 0 8px 30px rgba(0,0,0,.12); } }
</style></head>
<body>
...
// Charge un fichier same-origin (police ou image) et le convertit en data-URL.
...
export async function generateBpuB2B2EPdf(cfg: BpuB2B2EConfig): Promise<void> {
  ...
  const html = buildBpuB2B2EHtml(cfg, { regular, medium, semibold }, { logoDark, logoLight });
